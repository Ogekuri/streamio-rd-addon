...
class AllDebrid(BaseDebrid):
    ...
    def add_magnet(self, magnet, ip):
        url = f"{self.base_url}magnet/upload?agent=streamiordaddon&apikey={self.config['debridKey']}&magnet={magnet}&ip={ip}"
        return self.get_json_response(url)

    def add_torrent(self, torrent_file, ip):
        url = f"{self.base_url}magnet/upload/file?agent=streamiordaddon&apikey={self.config['debridKey']}&ip={ip}"
    ...
    def check_magnet_status(self, id, ip):
        url = f"{self.base_url}magnet/status?agent=streamiordaddon&apikey={self.config['debridKey']}&id={id}&ip={ip}"
        return self.get_json_response(url)

    def unrestrict_link(self, link, ip):
        url = f"{self.base_url}link/unlock?agent=streamiordaddon&apikey={self.config['debridKey']}&link={link}&ip={ip}"
        return self.get_json_response(url)
    ...
    def get_availability_bulk(self, hashes_or_magnets, ip=None):
        ...
        url = f"{self.base_url}magnet/instant?agent=streamiordaddon&apikey={self.config['debridKey']}&magnets[]={'&magnets[]='.join(hashes_or_magnets)}&ip={ip}"
        return self.get_json_response(url)
